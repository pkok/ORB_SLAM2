...
#include "MsgSync/MonoMsgSynchronizer.h"
...
int main(int argc, char **argv)
{
    ...
    ORBVIO::MonoMsgSynchronizer msgsync(imageMsgDelaySec);
    ros::NodeHandle nh;
    ros::Subscriber imagesub;
    ros::Subscriber imusub;
    if(ORB_SLAM2::ConfigParam::GetRealTimeFlag())
    {
        imagesub = nh.subscribe(config._imageTopic, /*200*/ 2, &ORBVIO::MonoMsgSynchronizer::imageCallback, &msgsync);
        imusub = nh.subscribe(config._imuTopic, 200, &ORBVIO::MonoMsgSynchronizer::imuCallback, &msgsync);
    }
